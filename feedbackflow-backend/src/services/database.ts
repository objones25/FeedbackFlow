...
export class DatabaseService {
  ...
  public async findById<T extends QueryResultRow = QueryResultRow>(
    ...
    const result = await this.query<T>(`SELECT * FROM ${table} WHERE id = $1`, [
      id,
    ]);
  ...
  public async createFeedbackEntries(
    entries: Array<{
      sourceId: number;
      rawText: string;
      author: string;
    ...
    for (const entry of entries) {
      const result = await this.insert("feedback_entries", {
        ...
        external_id: entry.externalId, // Add this line
      });
  ...
  public async createFeedbackGroups(
    groups: Array<{
      name: string;
      description: string;
  ...
  public async getSentimentDistribution(
    timeframe: string
  ...
  public async getSentimentTrends(timeframe: string): Promise<
    Array<{
      date: string;
      positive: number;
      negative: number;
  ...
  > {
    try {
      ...
      return result.rows.map((row) => {
        const fallbackDate =
          new Date().toISOString().split("T")[0] || "2025-07-20";
        return {
  ...
  public async getTopFeedbackGroups(limit: number): Promise<
    Array<{
      id: number;
      name: string;
      description: string;
  ...
  > {
    ...
    return result.rows.map((row) => ({
      ...
      sentenceIds: Array.isArray(row.sentence_ids)
        ? (row.sentence_ids as number[])
        : [],
